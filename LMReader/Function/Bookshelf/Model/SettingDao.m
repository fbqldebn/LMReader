...
#import "SettingDao.h"
#define kFileName @"Reader.sqlite3"
...
-(void)saveOrUpdate:(NSString *)name value:(NSString *)value
{
    ...
    char *errorMsg;
	NSString * createSql=@"CREATE TABLE IF NOT EXISTS SETTING(NAME TEXT PRIMARY KEY ,VALUE TEXT );";
...
//		NSLog(@"fail creating table %s",errorMsg);
   ...
   // sqlite3_stmt *statement;
    NSString *sql=[[NSString alloc] initWithFormat:@"INSERT OR REPLACE INTO SETTING (NAME,VALUE)VALUES('%@','%@')",name,value];
...
-(NSString *)getValueByName:(NSString *)name ;
{
	...
	char *errorMsg;
	NSString * createSql=@"CREATE TABLE IF NOT EXISTS SETTING(NAME PRIMARY KEY,VALUE TEXT);";
	if (sqlite3_exec(database,[createSql UTF8String],NULL,NULL,&errorMsg)!=SQLITE_OK)
	{
		sqlite3_close(database);
		//NSAssert(0,errorMsg);
//		NSLog(@"fail creating table %s",errorMsg);
    ...
    NSString *selectSql=[[NSString alloc] initWithFormat:@"SELECT * FROM SETTING WHERE NAME='%@'",name];
	sqlite3_stmt *statement;
