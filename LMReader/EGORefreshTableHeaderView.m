...
- (void)setState:(EGOPullRefreshState)aState{
	...
	switch (aState) {
        ...
        {
            _statusLabel.text = NSLocalizedString(@"全部加载完成", @"全部加载完成");
			[_activityView stopAnimating];
...
- (void)egoRefreshScrollViewDidScroll:(UIScrollView *)scrollView {	
    ...
    if (_state == EGOOPullRefreshFinish) {
        return;
    }
...
- (void)egoRefreshScrollViewDidEndDragging:(UIScrollView *)scrollView {
	...
	if (scrollView.contentOffset.y + (scrollView.frame.size.height) + 100> scrollView.contentSize.height + RefreshViewHight && !_loading) {
